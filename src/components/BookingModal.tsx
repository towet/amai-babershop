...
export const BookingModal = ({ isOpen, onClose }: BookingModalProps) => {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'date' && selectedBarberId) {
      ...
      // Check if any other barbers are available at this time slot
      await checkForAlternativeBarbers(selectedBarberId, formData.date, value);
    }
  };

  // Find alternative barbers when the selected barber has a conflict
  const checkForAlternativeBarbers = async (currentBarberId: string, date: string, time: string) => {
    // Clear previous results
    setConflictingBarbers([]);
    setAlternativeBarbers([]);

    try {
      // Check all barbers for availability at this time slot
      const available: Barber[] = [];
      const conflicting: string[] = [];

      for (const barber of barbers) {
        if (barber.id === currentBarberId) continue; // Skip current barber

        // Get available time slots for this barber on this date
        const timeSlots = await getAvailableTimeSlotsForBarber(barber.id, date);
        
        // If the selected time is available for this barber
        if (timeSlots.includes(time)) {
          available.push(barber);
        } else {
          conflicting.push(barber.id);
        }
      }

      setAlternativeBarbers(available);
      setConflictingBarbers(conflicting);
    } catch (error) {
      console.error('Error checking for alternative barbers:', error);
    }
  };
                        ...
                        {selectedBarberId && formData.date ? (
                          availableTimeSlots.length > 0 ? (
                            <div className="space-y-4">
                              ...
                              {/* Alternative barbers section */}
                              {formData.time && alternativeBarbers.length > 0 && (
                                <div className="p-3 sm:p-4 bg-gray-800/80 border border-gray-700 rounded-lg">
                                  <div className="flex items-start gap-2 sm:gap-3">
                                    <div className="w-full">
                                      <p className="text-white text-xs sm:text-sm font-medium mb-2 sm:mb-3">Switch to an available barber at this time:</p>
                                      <div className="mt-1 sm:mt-2 space-y-2">
                                        {alternativeBarbers.map(barber => (
                                          <div 
                                            key={barber.id} 
                                            className="flex items-center gap-2 sm:gap-3 text-gray-300 text-xs p-2 sm:p-3 bg-gray-700/60 hover:bg-gray-700/80 rounded-md transition-colors cursor-pointer"
                                            onClick={() => {
                                          ...
                                          >
                                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden flex-shrink-0 bg-gray-700 border border-gray-600">
                                              <img 
                                                src={barber.photoUrl} 
                                                alt={barber.name}
                                                className="w-full h-full object-cover" 
                                            ...
                                            <div className="flex-1 min-w-0">
                                              <p className="font-medium text-xs sm:text-sm truncate">{barber.name}</p>
                                              <p className="text-gray-400 text-xs mt-0.5 truncate">{barber.specialty}</p>
                                            ...
                                            <button 
                                              type="button"
                                              className="px-2 sm:px-3 py-1 sm:py-1.5 bg-orange-600 hover:bg-orange-500 text-white rounded text-xs transition-colors flex-shrink-0"
                                            >
                                              Select
                                            </button>
                                          </div>
