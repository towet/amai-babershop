...
export const HeroSection = () => {
  ...
  const defaultProfiles = [
      { id: 1, name: "", rating: 4.9, distance: "Amai", fallbackImage: "https://images.unsplash.com/photo-1605497788044-5a32c7078486?q=80&w=1374&auto=format&fit=crop" },
      { id: 2, name: "", rating: 4.8, distance: "Amai", fallbackImage: "https://images.unsplash.com/photo-1567894340315-735d7c361db0?q=80&w=1374&auto=format&fit=crop" },
      { id: 3, name: "", rating: 5.0, distance: "Amai", fallbackImage: "https://images.unsplash.com/photo-1517832606299-7ae9b720a186?q=80&w=1374&auto=format&fit=crop" },
  ];

  const defaultStyles = [
      { id: 1, label: "Fade", fallbackImage: "https://images.unsplash.com/photo-1622286342621-4bd786c2447c?q=80&w=1470&auto=format&fit=crop" },
      { id: 2, label: "Classic", fallbackImage: "https://images.unsplash.com/photo-1519421052252-d4de633a4613?q=80&w=1374&auto=format&fit=crop" },
      { id: 3, label: "Modern", fallbackImage: "https://images.unsplash.com/photo-1567894340315-735d7c361db0?q=80&w=1374&auto=format&fit=crop" },
  ];

  useEffect(() => {
    const fetchHeroImages = async () => {
      const imageKeys = ["barber1.jpg", "barber2.jpg", "barber3.jpg"];
      const urls = await Promise.all(
        imageKeys.map(async (key) => {
          const { data } = await supabase.storage.from('heroimage').getPublicUrl(key);
          return data?.publicUrl ? `${data.publicUrl}?t=${new Date().getTime()}` : '';
        })
      );
      setHeroImages(urls.filter(url => url));
    };
    fetchHeroImages();
  }, []);

  const barberProfiles = defaultProfiles.map((profile, index) => ({
    ...profile,
    image: heroImages[index] || profile.fallbackImage
  }));

  const barberStyles = defaultStyles.map((style, index) => ({
    ...style,
    image: heroImages[index] || style.fallbackImage
  }));
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-8">
              <div className="relative z-20">
                <h1 className="font-['Lato'] text-4xl md:text-5xl font-light tracking-[0.2em] text-white uppercase">
                  <span className="text-orange-400">Amai</span> Men's Care
                </h1>
                <p className="font-['Bebas_Neue'] text-5xl md:text-7xl text-white uppercase mt-2">
                  <span className="text-orange-400">Premium</span> Grooming <span className="text-orange-400">Experience</span>
                </p>
              </div>
