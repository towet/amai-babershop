...
export const BarberFinancialReport = () => {
  ...
  const totalRevenueGenerated = data.reduce((acc, item) => acc + item.totalRevenue, 0);
              ...
              <TableBody>
                {data.length > 0 ? data.map((item) => (
                  <TableRow key={item.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="text-right text-green-600 font-bold">{item.barberCommission.toFixed(2)}</TableCell>
                  </TableRow>
                )) : (
                  <TableRow>
