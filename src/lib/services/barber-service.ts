...
// Deletes a barber and all their related data (appointments, reviews, etc.)
export const deleteBarber = async (barberId: string): Promise<void> => {
  try {
    ...
    if (error) {
      ...
      throw new Error(`Failed to delete barber: ${error.message}`);
    }
  } catch (error) {
    console.error('Error in deleteBarber function:', error);
    throw error;
  }
};
