...
// Get all appointments for a specific barber (for statistics and dashboard)
export const getAllBarberAppointments = async (barberId: string): Promise<Appointment[]> => {
  try {
    console.log('Fetching ALL appointments for barber ID:', barberId);
    ...
    // Find all appointments for the barber without date filtering
...
// Get all upcoming appointments for a barber with full details
export const getUpcomingBarberAppointments = async (barberId: string): Promise<AppointmentWithDetails[]> => {
  try {
    // Get today's date in ISO format (YYYY-MM-DD)
    const today = new Date().toISOString().split('T')[0];
    console.log(`Getting upcoming appointments for barber ID ${barberId} from date ${today} onwards`);
    
    // First, try to find appointments without any status filter to see if there's data
    ...
    if (allError) {
      console.error('Error checking all upcoming appointments:', allError);
    } else {
      console.log(`Raw query found ${allData?.length || 0} total upcoming appointments for barber`);
      
      // Log the appointment IDs and dates for debugging
      if (allData && allData.length > 0) {
        console.log('Upcoming appointment data preview:', 
          allData.slice(0, 5).map(a => ({
            id: a.id,
            date: a.date,
            time: a.time,
            status: a.status,
    ...
    // Now get the appointments with proper filtering and ordering
    const { data, error } = await supabase
      .from('appointments')
      .select(`
        *,
